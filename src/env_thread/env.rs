...
use std::rc::Rc;
use super::{ State, Transition, EnvThreadMessage };
...
impl Env {
    pub fn new(receiver : Receiver<EnvThreadMessage>, game_thread_sender : Sender<GameThreadMessage>, plot_thread_sender : Sender<PlotThreadMessage>) -> Result<Env, StepError> {
    ...
    fn send_episode_score_to_plot_thread(&self, score : u32) {
        self.plot_thread_sender.send(PlotThreadMessage::Datum((f64::from(self.n_step), f64::from(score)))).unwrap();
    }
    pub fn step(&mut self, action : u8) -> Result<(), StepError> {
        let state_slice = Self::state_as_slice(&mut self.state).clone();
        let score = self.score;
        self.game_thread_sender.send(GameThreadMessage::Action(action)).unwrap();
        ...
        self.pending_transitions.push_back((state_slice, next_state_slice, action, reward, terminated));
        if terminated {
            self.next_game()?;
        }
    ...
    fn raw_wait_for_next_frame(receiver : &Receiver<EnvThreadMessage>, waiting_hold : &mut bool) -> Result<(ImageOwned2, u32), StepError> {
        loop {
    ...
    fn truncate(&mut self) -> Result<(), StepError> {
        self.game_thread_sender.send(GameThreadMessage::Truncation).unwrap();
        self.next_game()?;
    ...
    pub fn state(&self) -> State {
        <&State>::try_from(&*self.state.clone().make_contiguous()).unwrap().clone()
    }
