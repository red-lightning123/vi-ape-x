...
pub trait ArgTuple<const N : usize> {
    fn feed_to<'a>(&'a self, session_run_args : &mut SessionRunArgs<'a>, locations : &[ArgLocation; N]);
}
...
impl<T1 : Arg, T2 : Arg, T3 : Arg, T4 : Arg, T5 : Arg> ArgTuple<5> for (T1, T2, T3, T4, T5) {
    fn feed_to<'a>(&'a self, session_run_args : &mut SessionRunArgs<'a>, [loc1, loc2, loc3, loc4, loc5] : &[ArgLocation; 5]) {
        self.0.feed_to(session_run_args, loc1);
...
impl<T1 : Arg, T2 : Arg, T3 : Arg, T4 : Arg, T5 : Arg, T6 : Arg, T7 : Arg, T8 : Arg, T9 : Arg> ArgTuple<9> for (T1, T2, T3, T4, T5, T6, T7, T8, T9) {
    fn feed_to<'a>(&'a self, session_run_args : &mut SessionRunArgs<'a>, [loc1, loc2, loc3, loc4, loc5, loc6, loc7, loc8, loc9] : &[ArgLocation; 9]) {
        self.0.feed_to(session_run_args, loc1);
        self.1.feed_to(session_run_args, loc2);
        self.2.feed_to(session_run_args, loc3);
        self.3.feed_to(session_run_args, loc4);
