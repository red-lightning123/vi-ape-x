...
impl TrainingSchedule {
    pub fn new(eps_min : f64, eps_max : f64, n_eps_random_steps : u32, n_eps_greedy_steps : u32, target_update_interval_steps : u32) -> TrainingSchedule {
        TrainingSchedule {
            n_step: 0,
            eps_min,
            eps_max,
    ...
    pub fn eps(&self) -> f64 {
        let eps = self.eps_max + (self.eps_min - self.eps_max) * f64::from(self.n_step - self.n_eps_random_steps) / f64::from(self.n_eps_greedy_steps);
        if eps < self.eps_min {
            self.eps_min
        } else {
            eps
