...
impl HumanInterface<'_> {
    pub fn new<'a>(ref_win : &Window) -> HumanInterface<'a> {
        ...
        conn.create_window(ref_win.depth(), win, root_win, 0, 0, ref_win.width(), ref_win.height(), 0, ref_win.class(), screen.root_visual, &xproto::CreateWindowAux::default()).unwrap();
        let win_title = b"Feed";
        conn.change_property(xproto::PropMode::REPLACE, win, xproto::AtomEnum::WM_NAME, xproto::AtomEnum::STRING, 8, win_title.len() as u32, win_title).unwrap();
        conn.change_window_attributes(win, &xproto::ChangeWindowAttributesAux::default().event_mask(xproto::EventMask::KEY_PRESS | xproto::EventMask::KEY_RELEASE)).unwrap();
        ...
        let gl =
            unsafe {
                glow::Context::from_loader_function_cstr(
                    |name_str| {
                        x11::glx::glXGetProcAddressARB(name_str.as_ptr().cast::<u8>()).unwrap() as *const core::ffi::c_void
                    }
                )
            };
        ...
        unsafe { gl.buffer_data_u8_slice(glow::ARRAY_BUFFER, vertices_u8_slice, glow::STATIC_DRAW) };
    ...
    pub fn draw(&self) {
        ...
        unsafe { self.gl.uniform_2_f32(self.gl.get_uniform_location(self.shader_program.handle(), "center").as_ref(), 0.0, 0.0) };
        unsafe { self.gl.uniform_2_f32(self.gl.get_uniform_location(self.shader_program.handle(), "dims").as_ref(), width, height) };
        unsafe { self.gl.tex_image_2d(glow::TEXTURE_2D, 0, glow::RG as i32, self.frame.width() as i32, self.frame.height() as i32, 0, glow::RG, glow::UNSIGNED_BYTE, Some(self.frame.as_ref().data())) };
