...
impl Program {
    pub fn new(gl : &glow::Context, source : &ProgramSource) -> Result<Program, ProgramCreationError> {
    ...
    fn build_shader(gl : &glow::Context, shader_type : ShaderType, shader_source : &str) -> Result<glow::Shader, ProgramCreationError> {
        let shader_type_raw = shader_type.as_raw();
        let shader = unsafe { gl.create_shader(shader_type_raw).map_err(ProgramCreationError::GlError)? };
        unsafe { gl.shader_source(shader, shader_source) };
        unsafe { gl.compile_shader(shader) };
        if unsafe { !gl.get_shader_compile_status(shader) } {
            return Err(ProgramCreationError::CompilationFailed(shader_type, unsafe { gl.get_shader_info_log(shader) }));
        }
        Ok(shader)
    }
    
    fn link(gl : &glow::Context, shader_program : glow::Program) -> Result<glow::Program, ProgramCreationError> {
        unsafe { gl.link_program(shader_program) };

        if unsafe { !gl.get_program_link_status(shader_program) } {
            return Err(ProgramCreationError::LinkingFailed(unsafe { gl.get_program_info_log(shader_program) }));
        }
        Ok(shader_program)
    }
