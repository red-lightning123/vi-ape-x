...
pub fn spawn_ui_thread(receiver : Receiver<UiThreadMessage>, master_thread_sender : Sender<MasterThreadMessage>) -> std::thread::JoinHandle<()> {
    const VARIANT : UiImplVariant = UiImplVariant::Gl;
    const SPAWN_FN : fn(receiver : Receiver<UiThreadMessage>, master_thread_sender : Sender<MasterThreadMessage>) -> std::thread::JoinHandle<()> = match VARIANT {
