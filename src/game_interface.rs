...
fn choose_matching_fbconfigs(display : *mut x11::xlib::Display, screen_num : i32) -> *mut x11::glx::GLXFBConfig {
...
fn get_children(conn : &XCBConnection, win : u32) -> Result<Vec<u32>, x11rb::rust_connection::ReplyError> {
...
fn get_window_title_bytes(conn : &XCBConnection, win : u32) -> Result<Vec<u8>, x11rb::rust_connection::ReplyError> {
    Ok(conn.get_property(false, win, xproto::AtomEnum::WM_NAME, xproto::AtomEnum::STRING, 0, 32)?.reply()?.value)
}
...
fn get_window_instance_name(conn : &XCBConnection, win : u32) -> Result<String, x11rb::rust_connection::ReplyError> {
    Ok(std::str::from_utf8(WmClass::get(conn, win)?.reply()?.instance()).unwrap().to_string())
}
...
impl Window {
    fn new(handle : u32, dims : xproto::GetGeometryReply, attribs : xproto::GetWindowAttributesReply, image_len : usize) -> Window {
        Window {
            handle,
            dims,
            attribs,
...
impl VSyncData {
    fn new(display : &mut X11Display, conn : &XCBConnection, screen : &xproto::Screen, screen_num : i32, root_win : u32, ref_win : &Window) -> VSyncData {
        let win = conn.generate_id().unwrap();
        conn.create_window(ref_win.depth(), win, root_win, 0, 0, 1, 1, 0, xproto::WindowClass::INPUT_OUTPUT, screen.root_visual, &xproto::CreateWindowAux::default()).unwrap();
...
impl<'a> GameInterface<'a> {
    pub fn new() -> GameInterface<'a> {
        ...
        let conn = display.to_xcb_connection_mut().expect("Can't convert display to xcb connection");
        let screen_num = display.default_screen();
        let shm_extension_present = conn.extension_information(x11rb::protocol::shm::X11_EXTENSION_NAME).expect("Can't query extension information").is_some();
    ...
    fn wait_for_win_with_title(&self, title : &str) -> u32 {
        loop {
            if let Some(win_handle) = find_descendant_win_with_title(&self.conn, self.root_win, title) {
                return win_handle;
            }
        }
    }
    pub fn start(&mut self) {
        ...
        self.process = Some(Command::new("python").current_dir("pixaves_journey").args(["main.py"]).stderr(std::process::Stdio::null()).spawn().unwrap());
        let win_handle = self.wait_for_win_with_title(WIN_TITLE);
        let win_dims = self.conn.get_geometry(win_handle).unwrap().reply().unwrap();
        let win_attribs = self.conn.get_window_attributes(win_handle).unwrap().reply().unwrap();
        let win_image_len = (win_dims.depth as usize) * (win_dims.width as usize) * (win_dims.height as usize);
        let win = Window::new(win_handle, win_dims, win_attribs, win_image_len);
        let image_seg = self.image_seg.get_or_init(|| XShmSeg::new(&self.conn, win.image_len()));
        self.vsync.get_or_init(|| VSyncData::new(&mut self.display, &self.conn, &self.screen, self.screen_num, self.root_win, &win));
        ...
        while self.conn.shm_get_image(win.handle(), 0, 0, win.width(), win.height(), !0, xproto::ImageFormat::Z_PIXMAP.into(), image_seg.xid(), 0).unwrap().reply().is_err() {
        }
        self.win = Some(win);
    ...
    pub fn end(&mut self) {
        if let Some(process) = &mut self.process {
            ...
            self.conn.destroy_window(self.win.as_ref().unwrap().handle()).unwrap();
            process.kill().unwrap();
    ...
    pub fn next_frame(&mut self) {
        ...
        self.conn.shm_get_image(win.handle(), 0, 0, win.width(), win.height(), !0, xproto::ImageFormat::Z_PIXMAP.into(), image_seg.xid(), 0).unwrap().reply().unwrap();
    }
    ...
    pub fn get_current_frame(&self) -> ImageRef4 {
        ...
        let image_seg = self.image_seg.get().expect("get_current_frame no image seg");
        let full_window = ImageRef4::new(u32::from(win.width()), u32::from(win.height()), unsafe { std::slice::from_raw_parts(image_seg.address().cast::<u8>(), win.image_len()) });
        full_window
    }
    pub fn send(&mut self, key : GameKey, kind : KeyEventKind) {
        ...
        match kind {
            KeyEventKind::Press => {
                ...
                self.conn.send_event(false, win.handle(), xproto::EventMask::default(), event).unwrap();
            }
