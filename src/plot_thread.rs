...
impl Plot {
    ...
    fn draw_plot(&self) {
            let plot = PlotlibPlot::new(self.points.clone()).line_style(
                LineStyle::new()
                    .colour("blue")
    ...
    fn load<P : AsRef<Path>>(&mut self, path : P) {
        ...
        *self = bincode::deserialize_from(&mut file).unwrap();
        assert!(!has_data_left(file).unwrap(), "deserialization of file didn't reach EOF");
    }
...
impl Loop {
    fn new(receiver : Receiver<PlotThreadMessage>, master_thread_sender : Sender<MasterThreadMessage>) -> Loop {
        Loop {
            receiver,
            ...
            match mode {
                ThreadMode::Held => {
                    match self.receiver.recv().unwrap() {
                        PlotThreadMessage::Master(message) => {
                            match message {
                                MasterMessage::Save(path) => {
                                    self.save(path);
                                    self.master_thread_sender.send(MasterThreadMessage::Done(THREAD_ID)).unwrap();
                                }
...
pub fn spawn_plot_thread(receiver : Receiver<PlotThreadMessage>, master_thread_sender : Sender<MasterThreadMessage>) -> std::thread::JoinHandle<()> {
