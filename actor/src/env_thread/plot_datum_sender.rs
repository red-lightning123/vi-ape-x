...
impl PlotDatumSender {
    ...
    fn send_datum(&self, plot_type: PlotType, datum: f64, schedule: &ActorSchedule) {
        self.sender
    ...
    pub fn send_episode_score(&self, score: u32, schedule: &ActorSchedule) {
        self.send_datum(PlotType::EpisodeScore, f64::from(score), schedule);
    }

    pub fn send_loss(&self, loss: f32, schedule: &ActorSchedule) {
        self.send_datum(PlotType::Loss, f64::from(loss), schedule);
    }

    pub fn send_q_val(&self, q_val: f32, schedule: &ActorSchedule) {
